#!/usr/bin/env Rscript
# Mechanical check phases for the deploy-bgnormr skill. Run from the repo root.
#
#   Rscript .claude/skills/deploy-bgnormr/deploy.R <phase>
#
# Phases:
...
message("== deploy.R phase=", phase, "  logs -> ", out)
...
run_bioccheck <- function() {
  ...
  # the checkBiocCheckOutputFolder false-positive never fires.
  clean_bc <- function() unlink(list.files(".", pattern = "\\.BiocCheck$",
                                           full.names = TRUE), recursive = TRUE)
  clean_bc()
  # Mirror the CI invocation in .github/workflows/check-bioc.yml.
