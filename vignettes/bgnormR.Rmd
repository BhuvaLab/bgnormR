...
`inst/extdata/PA_HNC_sample.ome.tiff` and contains five markers across a
...
The same function also reads standard **OME-TIFF** and **OME-Zarr** (OME-NGFF)
images, auto-detecting the container from the path (OME-Zarr requires the
`Rarr` package).  Rich per-channel and slide metadata is organised into an
OME-style `QPTIFFMetadata` object, available via `metadata()` and summarised as
a tidy table by `channel_table()`:

```{r metadata}
meta <- metadata(img)
qpi_format(meta)     # e.g. "fusion_paged", "ome_tiff", "ome_zarr"
channel_table(meta)[, c("name", "fluorophore", "exposure_time_us")]
```

The native, Java-free TIFF/QPTIFF reader and writer implemented here was
translated from the [bioio-tifffile fork](https://github.com/rtubelleza/bioio-tifffile/tree/feature/read-qptiffs-rich)
by Rafael Tubelleza.
...
p_raw <- plot_qptiff(img, markers = c("PanCK", "Vimentin", "CD20")) +
  labs(title = "Raw")
p_raw
```
...
p_bg <- plot_qptiff(res, markers = c("PanCK", "Vimentin", "CD20"), scale = "marker") +
  labs(title = "Adjusted (bgnorm)")
p_bg
```
...
p_bgq <- plot_qptiff(res_q, markers = c("PanCK", "Vimentin", "CD20"), scale = "marker") +
  labs(title = "Adjusted (bgnormQ)")
p_raw
p_bg
p_bgq
```
